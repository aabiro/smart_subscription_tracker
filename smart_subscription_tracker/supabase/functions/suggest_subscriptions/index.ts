...
interface SuggestSubscriptionItem {
  name: string;
  description: string;
  price: number;
...
interface OpenAIResponseFormat {
  suggestions: SuggestSubscriptionItem[];
}
...
Ensure the output is ONLY this JSON object and nothing else.`;
    ...
    try {
      ...
      // to ensure they match SuggestSubscriptionItem structure, but for now, we assume the model follows the prompt.
    } catch (e) {
      console.error(
